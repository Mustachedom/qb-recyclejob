...
local uhohs = {}
local Sales, Stock, salesLoc = {}, {}, vector3(1048.224, -3097.071, -38.999)
...
local function checkStock(item, amount)
    if not Config.LimitedMaterials then return true end
    if Stock[item] >= amount then
        return true
    else
        TriggerClientEvent('QBCore:Notify', src, QBCore.Shared.Items[item].label .. ' Is Out Of Stock!', 'error')
        return false
    end
end
...
local function getItem(source, item, amount)
    local Player = QBCore.Functions.GetPlayer(source)
    if Config.LimitedMaterials then
        if not checkStock(item, amount) then return end
